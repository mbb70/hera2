...
export const currentFilteredMatches = createSelector(
  ...
  (matches, searchText, players) => {
    ...
    return matches.filter((m) => {
      const p1Match = vu.caseInsIncludes(players.getIn([m.get('p1'), 'name']), searchText);
      const p2Match = vu.caseInsIncludes(players.getIn([m.get('p2'), 'name']), searchText);
...
export const currentFilteredPlayers = createSelector(
  [ currentPlayers, getSearchText, getSortByScore, currentSettings, getHideDropped ],
  (players, searchText, sortByScore, settings, hideDropped) => {
    return players.filter((p) => {
      ...
      return (searchEmpty || searchMatches) && !isBye && !deleted && !(dropped && hideDropped);
    }).valueSeq().sortBy(p => {
      if (sortByScore) {
