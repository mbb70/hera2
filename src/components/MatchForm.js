...
class MatchForm extends PureComponent {
  ...
  handleScoreSelection = (i) => {
    this.setState({ match: {...this.state.match, score: this.getScoreFromIndex(i) } });
  }
  ...
  render() {
    ...
    const entryPoint = <MatchCard match={this.props.match} players={this.props.players}/>;
    ...
    const invalid = winnerSelected === -1 || (winnerSelected !== 1 && scoreSelected === -1);
    const drop = (match.drop || []).map(pId => pId === p1.id ? 0 : 1);
    return (
      <BasicFormModal {...{entryPoint, header, submitText, resetForm, onFormSubmit, invalid}}>
        <FormGroup>
          <Label>Winner</Label>
          <ButtonCol selected={winnerSelected} onClick={this.handleWinnerSelection}>
            <Button type='button' color='success'>{p1.name}</Button>
            <Button type='button' color='secondary'>Draw</Button>
            <Button type='button' color='success'>{p2.name}</Button>
          </ButtonCol>
        </FormGroup>
        {match.winner !== -1 && (
          <FormGroup>
            <Label>Score</Label>
            <ButtonRow selected={scoreSelected} onClick={this.handleScoreSelection}>
              <Button type='button' color='info'>2 - 0</Button>
              <Button type='button' color='secondary'>2 - 1</Button>
              <Button type='button' color='info'>1 - 0</Button>
            </ButtonRow>
          </FormGroup>
        )}
        <FormGroup>
          ...
          <ButtonRow multi selected={drop} onClick={this.handleDropSelection}>
            <Button type='button' color='danger'>Drop {p1.name}</Button>
            <Button type='button' color='danger'>Drop {p2.name}</Button>
          </ButtonRow>
        </FormGroup>
      </BasicFormModal>
    );
