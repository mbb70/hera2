...
class AddPlayerForm extends Component {
  ...
  render() {
    ...
    return (
      <BasicFormModal {...{entryPoint, header, submitText, resetForm, onFormSubmit}}>
        <FormGroup>
          <Label>Names</Label>
          {this.state.names.map((name, i) =>
            <Input autoFocus={this.state.names.length === 1} key={i} data-key={i} type="text" value={name} onChange={this.handleKeyPress}/>
          )}
        </FormGroup>
      </BasicFormModal>
    );
