...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case a.ADD_PLAYERS: {
      ...
      return saveState(
        state.mergeIn(['players'], newPlayers).set('maxPlayerId', maxPlayerId)
      );
    }
    case a.PAIR_PLAYERS: {
    ...
    case a.SAVE_SETTINGS: {
      ...
      return saveState(
        state
          .mergeIn(
            ['settings'],
            fromJS({ [currentTournament]: action.settings })
          )
          .mergeIn(
            ['tournaments'],
            fromJS({
              [currentTournament]: action.settings.tournamentName || tName,
            })
          )
      );
    }
    case a.UPDATE_PLAYER: {
      return saveState(
        state.update('players', players =>
          players.map(
            (p, pId) =>
              pId === action.player.id ? p.merge(fromJS(action.player)) : p
          )
        )
      );
    }
    case a.UPDATE_MATCH: {
      return saveState(
        state.update('matches', matches =>
          matches.map(
            (m, matchId) =>
              matchId === action.match.id ? m.merge(fromJS(action.match)) : m
          )
        )
      );
    }
